...
#    Author: Karl Vietmeier - Cloud Solutions Architect VAST Data Inc.
#
#    Install: 
...
#       Linux on ARM64 / aarch64 (Lima, Apple Silicon virtualization, etc.)
#
# Usage:
#   ./install_cloud_sdks.sh [--quiet|--verbose] [--force]
# Flags:
#   --quiet: Suppress command output (default)
#   --verbose: Show command output
#   --force: Force re-installation even if the tool already exists
#
...
command_exists() {
    if [ "$FORCE" = true ]; then
        return 1 # Pretend it doesn't exist so it forces install
    fi
...
install_prereqs() {
    ...
    STATUS[prereqs]="Installed/Verified"
}
...
# -------------------------
# AWS (With Multi-Arch Auto-Detection)
# -------------------------
install_aws() {
    if ! command_exists aws; then
        ...
        TMP_DIR=$(mktemp -d)
        cd "$TMP_DIR"
        run_cmd curl -s "$AWS_URL" -o awscliv2.zip
...
install_gcp() {
    if ! command_exists gcloud; then
        if [[ "$PKG_MGR" == "apt" ]]; then
            ...
            STATUS[gcloud]="Skipped (Non-apt system)"
        fi
    else
        STATUS[gcloud]="Already Installed"
    fi
}
...
install_terraform() {
    if ! command_exists terraform; then
        # Bypass prompt if running in quiet mode or forced mode
...
main() {
    ...
    printf "%-15s %-20s\n" "Tool" "Status"
    echo "-------------------------------------"
    for tool in wslu prereqs azure aws gcloud oci terraform asciinema; do
        printf "%-15s %-20s\n" "$tool" "${STATUS[$tool]}"
    done
    echo "====================================="
    echo "✅ Done!"
