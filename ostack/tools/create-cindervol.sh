...
# Create the volumes
for vol in $(seq -f "%03g" 1 10)
   do
     echo "`date` = cinder create 100 --display-name ${vol_name}${vol} --volume-type $vol_type"
     cinder create 1000 --display-name ${vol_name}${vol} --volume-type $vol_type &> /dev/null
   done
sleep 1
